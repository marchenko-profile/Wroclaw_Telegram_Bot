...
bot_token = '5993050156:AAHMtyE7zz53blDPqJ1O1GKEdhcAzXFmOcs'
# Здесь указываем ваш Telegram-идентификатор
chat_id = '-1001978822090'

# Create a bot instance
bot = telebot.TeleBot(bot_token)

# Define the base URL for the website
base_url = 'https://www.wroclaw.pl/dla-mieszkanca/aktualnosci'
...
def check_for_new_ads():
    ...
    url = 'https://www.wroclaw.pl/dla-mieszkanca/aktualnosci'
    ...
    if response.status_code == 200:
        ...
        ad = soup.find('li', {'class': 'boxWithImg'})
...
def parse_ads():
    ...
    if response.status_code == 200:
        ...
        ads = soup.find_all('li', {'class': 'boxWithImg'})
        ...
        for ad in ads:
            try:
                ...
                title = ad.find('h3', {'class': 'photoLnk__title'}).text
                ad_url = ad.find('a')['href']
                ...
                # Create an InlineKeyboardMarkup object for the button
                keyboard = types.InlineKeyboardMarkup()
                if ad_url:
                    button = types.InlineKeyboardButton(text='Otwórz wiadomość', url=ad_url)
                    keyboard.add(button)

                # Remove the link from the ad
                message_without_link = message.replace(ad_url, '')

                # Send the message to Telegram with photos, title, description, and button
...
def is_ad_parsed(ad_url):
    with open('parsed_ads.txt', 'r') as file:
...
def mark_ad_as_parsed(ad_url):
    with open('parsed_ads.txt', 'a') as file:
        file.write(ad_url + '\n')
